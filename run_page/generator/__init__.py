...
class Generator:
    ...
    def sync_from_data_dir(self, data_dir, file_suffix="gpx", activity_title_dict={}):
        ...
        for t in tracks:
            created = update_or_create_activity(
                ...
                match_by_start=file_suffix == "gpx",
            )
            if created:
                sys.stdout.write("+")
            else:
    ...
    def load(self):
        # Older versions invented routes for route-less indoor workouts and
        # persisted them in the cache. Remove only those unverified legacy
        # values; HealthKit-repaired rows carry a source_id and are preserved.
        synthetic_indoor_routes = (
            self.session.query(Activity)
            .filter(
                func.lower(Activity.subtype) == "indoor",
                Activity.source_id.is_(None),
                Activity.summary_polyline.isnot(None),
                Activity.summary_polyline != "",
            )
            .all()
        )
        for activity in synthetic_indoor_routes:
            activity.summary_polyline = ""
        if synthetic_indoor_routes:
            print(
                f"Removed {len(synthetic_indoor_routes)} unverified synthetic "
                "indoor routes"
            )
            self.session.commit()
        ...
        for activity in activities:
            ...
            if not IGNORE_BEFORE_SAVING:
                activity_data["summary_polyline"] = filter_out(
                    activity.summary_polyline  # type: ignore
                )
